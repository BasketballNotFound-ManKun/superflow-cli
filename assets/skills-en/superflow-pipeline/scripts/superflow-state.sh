...
cmd_scale() {
  ...
  risk_hits="$(grep -REi 'api|api|curl|SQL|database|Mapper|XML|cross-repo|status field|enum|real entry|payment|refund|MQ|scheduled job|third party|SDK' "$CHANGE_DIR"/*.md "$CHANGE_DIR"/**/*.md 2>/dev/null | wc -l | tr -d ' ')"
