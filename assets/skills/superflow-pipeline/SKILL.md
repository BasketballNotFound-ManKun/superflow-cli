...
- **SQL 必须在合同详设阶段冻结**：只要需求涉及表、字段、索引、约束、初始化
  数据或历史数据迁移，`$superflow-docs` 必须先生成 `release-sql.md`。文件必须包含
  `database_change: true`、无占位符的精确 `target_sql_path`、
  `copy_policy: verbatim`、归一化 SQL 正文的 `sql_sha256`，以及唯一一段完整可执行
  `sql` 代码块。前置检查、DDL/DML、迁移、后置对账、默认值、约束和索引必须在
  此阶段定稿。
- **实现 Agent 只有复制权**：研发 prompt 必须引用 `release-sql.md`，把唯一 SQL
  代码块原样复制到 `target_sql_path` 并核对 SHA-256。不得在实现阶段新增、删除、
  重排、拆分或“优化”SQL；发现设计遗漏必须停止并退回 `$superflow-docs`，刷新
  handoff 后再继续。
- **禁止路径和 SQL 占位符**：`testN.sql`、`{version}.sql`、`待定路径`、伪 SQL、
  只有字段表格而无完整脚本均不得通过 docs/design guard。
...
- **只落一个冻结目标 SQL 文件**：同一需求的表、字段、索引、默认值和初始化数据
  必须已在 `release-sql.md` 合并冻结；实现 Agent 只能原样复制到唯一
  `target_sql_path`，禁止各任务另建或追加 SQL。
