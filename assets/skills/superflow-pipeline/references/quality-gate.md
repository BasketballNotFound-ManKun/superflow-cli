...
| 47   | **金额精度边界**                   | **涉及金额、费用、优惠、抵扣、退款、分账、支付、发票、余额、电费、服务费、套餐结算、比例分摊、明细分配、对账或财务展示时强制** | Superpowers 技术详设包含 `Money Precision Boundary` 及金额字段、计算态来源、中间精度、舍入边界、scale/rounding mode、分配/对账规则、禁止提前舍入点和测试证据；必须证明计算态金额与结算/展示态金额分离，中间态不得提前 `setScale(2)` 后继续切片、分摊、汇总或抵扣；测试必须覆盖半分、尾差或多明细分配场景 |
