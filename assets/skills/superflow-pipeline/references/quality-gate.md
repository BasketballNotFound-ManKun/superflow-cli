...
| 41   | **Superpowers 技术详设（HOW）**    | **完整 SDD 实现任务强制**                                                               | `docs/superpowers/specs/YYYY-MM-DD-*-technical-design.md` 存在并链接当前 handoff 上下文包；Superpowers 接管源码级 HOW、TDD/RED 切入点、团队/Reviewer/Tester 分工、worktree/端口计划和执行风险；不得覆盖 OpenSpec/SDD 的需求、API、DB、SQL、字段语义、tests 或验收门禁                                                          |
...
| 44   | **上下文防漂移 handoff**           | **所有进入实现的任务强制**                                                              | `.sdd/handoff/sdd-context.md`、`.json`、`.sha256` 和 `.sdd/state.yaml` 已生成；handoff_hash 仅在 .sdd/state.yaml 绑定并与上下文包及当前合同一致；正文链接上下文包；`superflow-guard.sh <change-dir> docs` 通过                                                                                                                      |
...
| 46  | **handoff hash 继承（阻塞级）**                | implementation prompt 必须包含 `上下文防漂移与状态继承`，链接 `.sdd/handoff/sdd-context.md`，引用 state.yaml 单点绑定，并要求修改冻结合同后（任务勾选与测试报告回填除外）重跑 `superflow-handoff.sh --refresh` 和 `superflow-guard.sh <change-dir> implement`                                                          |
...
| 18  | **上下文防漂移收口** | 检查 `.sdd/handoff/*`、`.sdd/state.yaml`、prompt、test-report     | state.yaml 与当前合同的 handoff_hash 一致；`superflow-guard.sh <change-dir> implement` 或 `verify` 通过；换会话/并行窗口后的执行报告证明已重读 handoff 和原始文档 | Leader                   |
