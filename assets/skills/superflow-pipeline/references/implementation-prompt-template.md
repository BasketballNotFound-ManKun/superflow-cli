...
| 金额字段 | 计算态来源 | 中间精度 | 舍入边界 | scale/rounding mode | 分配/对账规则 | 禁止提前舍入点 | 半分/尾差/多明细测试 |
| -------- | ---------- | -------- | -------- | ------------------- | ------------- | -------------- | -------------------- |
| \_\_\_\_ | \_\_\_\_ | \_\_\_\_ | \_\_\_\_ | \_\_\_\_ | \_\_\_\_ | \_\_\_\_ | TC-\_\_\_\_ |

计算态必须保留原始费率、数量、乘积、比例和分配精度，只能在已确认的结算、
持久化、支付、开票、导出或展示边界统一舍入。禁止先 `setScale(2)` 再继续切片、
分摊、汇总、抵扣或反推单价。多明细分配必须确定性处理尾差，并证明原始金额、
优惠金额、实付金额和分配合计满足合同恒等式；缺少证据时停止并报告 `Blocked`。
