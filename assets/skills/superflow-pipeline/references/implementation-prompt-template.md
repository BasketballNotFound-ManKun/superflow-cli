...
涉及数据库变更时，SQL 已在 [release-sql.md](../release-sql.md) 详设阶段冻结。
研发 Agent 只有原样复制权：按 front matter 的 `target_sql_path` 落盘，统一 LF 且
保留一个结尾换行，核对 `sql_sha256`。禁止新增、删除、重排、拆分或优化 SQL；
发现遗漏立即停止并退回 `$superflow-docs`，不得在本 prompt 内补设计。
...
api.md / sdd-quality-gate.md / Superpowers 技术详设复制 `外部集成配置与部署合同`：
...
环境相关配置和服务端资源不得只硬编码在注解、常量或业务代码中。测试自动创建、
测试历史资源存在或应用本地启动成功，不能证明生产已就绪；缺少生产创建 owner、
创建时点、就绪证据或回滚方式时，必须停止并报告 `Blocked`。

### 1.2.1 并发与幂等归属

如果本批涉及并发、批量下发/开通/续费、重复提交/回调/消费或重复外部调用，必须
从 sdd-quality-gate.md / Superpowers 技术详设复制 `Concurrency And Idempotency
Ownership` / `并发与幂等归属`：
...
默认使用应用层原子占用和稳定业务幂等键，在短事务内持久化 `PENDING` 后提交，释放
数据库锁再调用外部系统；重试复用原业务编码，结果不确定时阻止重复并进入对账。
禁止“先查再插”、单进程锁、随机单号或跨外部调用的长事务。唯一索引不是默认方案，
只有自然唯一、历史数据清理、NULL/软删除和冲突处理合同明确后才可作为可选兜底。
...
1. 阅读 [冻结发布 SQL](../release-sql.md)，核对精确 `target_sql_path`、
   `copy_policy: verbatim` 和 `sql_sha256`。
...
4. 如果发现缺失（字段不存在、默认值不对、索引缺失、初始化数据未插入），只能
   原样复制 `release-sql.md` 的唯一 SQL 代码块到目标版本文件并执行。冻结 SQL 本身
   有遗漏时必须返回详设阶段，禁止研发 Agent 追加语句。
...
- 禁止自行设计或创建额外 SQL；只允许把 `release-sql.md` 原样复制到冻结目标路径。
- 禁止把 SQL 写在临时说明、Agent 回复或独立任务文件中；任何变更先回写
  `release-sql.md`、重算哈希并重新通过 docs/design guard。
...
|---|---|---|---|---|---|---|---|
| Pxx | table_name | column_name | 类/Mapper/方法 | target_sql_path:行号或注释块 | 已存在/缺失/差异 | 已存在/缺失/差异 | 原样复制/退回docs/不采纳说明 |
```
...
10. **执行版本总 SQL 收口对账**：源码/Mapper 读写字段、开发库结构、测试库现状 + 冻结 SQL 最终结构必须一致；发现漏字段/漏索引/漏初始化数据时停止并退回 docs
...
- 禁止自行创建或追加 SQL，只允许原样复制 `release-sql.md` 到冻结目标路径
...
17. ❌ **实现阶段自行创建、追加或改写 SQL**
   - 错误：每个任务各建一个 SQL 文件，或者把 SQL 写在临时说明里不追加到汇总文件
   - 正确：退回 docs 更新 `release-sql.md`、哈希和 handoff；实现阶段只原样复制到 `target_sql_path`
