...
运行验收必须使用 tests.md 冻结的环境，不能擅自以本地容器或 Mock 替代共享开发环境。
用户要求全部接口实测时，先冻结“接口 × 角色 × 场景”矩阵，共用一次应用启动和测试账号，
逐项记录脱敏请求、响应文件、数据库预期集合与实际集合；遗漏、失败和阻塞单独列出。
进程健康且代码/配置未变化时复用同一实例，不为每个接口重启；必要重启记录原因。
开发任务仅以约定的开发环境验收，后续发布环境不得自动升级为当前开发交付门禁。
...
| 用例 ID | 恒等式/权威总额 | 币种/最小单位 | 差额反推 | 舍入前/后 | 策略/尾差接收方 | 正数/零/退款/并列余数 | 对账结论 |
| ------- | ---------------- | ------------- | -------- | ----------- | ----------------- | --------------------- | -------- |
| TC-M01  | 原始=优惠+实付/原始金额 | CNY/分 | 实付=原始-优惠 | ____/____ | 最大余数/____ | ____ | 一致/阻塞 |

必须证明计算态在最终边界前未提前舍入，并证明原始金额、优惠金额、实付金额、退款金额
或分配合计满足设计合同。存在权威总额时，还必须证明最后一个组成项确实由权威总额
减去其余组成项合计得到，而不是所有组成项分别计算和舍入。只验证最终展示两位
小数，不足以证明金额精度正确。证据还必须包含币种、渠道最小单位、舍入前值、
舍入后值、尾差接收方，以及正数、零、负数/退款、并列余数和幂等验证。汇率场景
另附 base/quote、汇率来源和目标结算舍入证据。
