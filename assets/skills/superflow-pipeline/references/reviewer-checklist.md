...
| # | 检查项 | 检查方法 | 通过标准 |
|---|---|---|---|
| 55.1 | 负向输入是否真实到达 | 对照 `tests.md` 的原始载荷、客户端命令与服务端日志/分支 | 畸形 JSON、错误 Content-Type、越权身份或非法字段以真实线协议注入，并证明命中目标解析/校验分支；不能只因状态码相同而通过 |
| 55.2 | 任务资源身份闭环 | 对照 owner 记录、资源记录、清理代码与失败用例 | 清理逐项比对 nonce、PID、端口、签名等冻结身份；任一不匹配拒绝清理且有证据。复用认证 helper 时只核对完整性、任务参数、一次失败清理和最终零残留 |
...
| 62 | 币种与最小单位 | 检查 DTO、适配器、支付渠道和数据库 | 未默认两位小数；内部 scale、ISO/渠道最小单位及转换边界明确，整数转换有溢出校验 |
| 63 | 舍入策略来源 | 检查逐行/整单/发票/支付边界 | 每个舍入点有层级、scale、mode 和合同来源，未全局硬编码同一策略 |
| 64 | 最大余数与稳定排序 | 检查多明细分配 | 高精度理想份额量化后逐最小单位分配尾差，同余数按稳定业务键排序，不依赖数据库自然顺序 |
| 65 | 负数与退款对称性 | 检查退款、冲正和负数分摊 | 正数、零、负数/退款均有证据，明确 `CEILING`/`FLOOR` 等模式的符号差异 |
| 66 | 汇率路径与元数据 | 检查跨币种换算 | base/quote、来源、时间、精度和唯一换算路径明确，只在目标结算边界舍入 |
