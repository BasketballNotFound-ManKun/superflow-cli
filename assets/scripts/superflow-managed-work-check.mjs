...
if (!taskId) {
  console.error(
    "用法: superflow-managed-work-check.mjs <project-root> <task-id>",
  );
  process.exit(2);
}
...
if (state.contractHash !== task.contractHash)
  fail("任务合同哈希与运行状态不一致");
...
if (deliveryStatuses.has(state.status)) {
  ...
  const executor = JSON.parse(
    fs.readFileSync(state.lastExecutorResult, "utf8"),
  );
  const review = JSON.parse(fs.readFileSync(state.lastReviewResult, "utf8"));
  verifyAcceptanceReview(task, runDir);
  ...
  if (
    review.result !== "pass" ||
    review.findings?.some((item) => item.blocking)
  ) {
    fail("交付状态与最终检查结论不一致");
  }
...
function verifyContract(contract) {
  ...
  if (contract.taskPrompt) {
    ...
    const promptHash = crypto
      ...
      .digest("hex");
    if (promptHash !== contract.taskPrompt.sha256)
      fail("冻结 Prompt 快照哈希不一致");
  }
  verifyAcceptanceContractSnapshot(contract);
  const immutable = {
...
function verifyAcceptanceContractSnapshot(contract) {
  ...
  const snapshot = path.join(taskDir, "acceptance-contract.json");
  const projection = path.join(taskDir, "acceptance-contract.md");
  if (!fs.existsSync(snapshot) || !fs.existsSync(projection)) {
    fail("冻结验收合同快照或投影缺失");
  }
  const value = JSON.parse(fs.readFileSync(snapshot, "utf8"));
  const acceptanceHash = crypto
    .createHash("sha256")
    .update(JSON.stringify(contract.acceptanceContract))
    .digest("hex");
  if (
    value.schemaVersion !== 1 ||
    value.taskId !== contract.taskId ||
    value.contractHash !== contract.contractHash ||
    value.acceptanceContractHash !== acceptanceHash ||
    JSON.stringify(value.acceptanceContract) !==
      JSON.stringify(contract.acceptanceContract)
  ) {
    fail("冻结验收合同快照与任务合同不一致");
  }
}

function verifyAcceptanceReview(task, runDir) {
  if (!task.acceptanceContract) return;
  const firstReviewFile = path.join(runDir, "review-result-1.json");
  if (!fs.existsSync(firstReviewFile)) {
    fail("冻结验收合同缺少首轮 Host 评审结果");
  }
  const review = JSON.parse(fs.readFileSync(firstReviewFile, "utf8"));
  const expected = acceptanceReferences(task.acceptanceContract);
  const reviewed = review.acceptanceCoverage?.reviewed;
  if (!Array.isArray(reviewed) || new Set(reviewed).size !== reviewed.length) {
    fail("最终 Host 评审缺少唯一验收合同覆盖记录");
  }
  ...
  for (const finding of review.findings ?? []) {
    if (
      ...
      fail(`finding ${finding.id ?? "unknown"} 未映射有效验收合同条目`);
    }
  }
}

function acceptanceReferences(contract) {
  return [
  ...
  while (current.startsWith(projectRoot)) {
    ...
    if (fs.existsSync(tasksFile)) {
      ...
      if (localPending.length > 0) {
        fail(
          `OpenSpec tasks.md 仍有 ${localPending.length} 个本地可执行任务未完成`,
        );
      }
      ...
      for (const item of tasks.filter(
        ...
        if (
          ...
          !proof.evidencePaths.every((item) =>
            taskEvidencePathExists(task, item),
          ) ||
          ...
          !proof.changedFiles.every((file) =>
            executor.changedFiles?.includes(file),
          )
        ) {
...
function verifyDeliveryStatus(status, completion) {
  if (
    ...
    completion.environmentPending.length === 0
  ) {
...
function verifyExecutorEvidence(task, result, review, runDir) {
  ...
  if (task.profile === "engineering" || task.profile === "sdd") {
    const successful = loadExecutorCommands(runDir).filter(
...
function verificationCategories(command = "") {
  ...
  if (
    ...
    /gradle\w*\s+test/.test(value)
  )
    categories.push("test");
  if (
    /(?:npm|pnpm|yarn)\s+(?:run\s+)?build/.test(value) ||
    ...
    /cargo\s+build|go\s+build/.test(value)
  )
    categories.push("build");
  if (
    /(?:npm|pnpm|yarn)\s+(?:run\s+)?(?:start|dev|serve)/.test(value) ||
    /spring-boot:run|docker\s+compose\s+up|(^|\s)(curl|wget)(\s|$)/.test(
      value,
    ) ||
    /health(?:check)?|integration|e2e/.test(value)
  )
