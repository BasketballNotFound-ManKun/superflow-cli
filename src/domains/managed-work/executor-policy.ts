...
const STABLE_POLICY_LINES = [
  ...
  "File edits must invoke the apply_patch CLI through exec_command with a heredoc (cmd: \"apply_patch <<'PATCH' ... PATCH\"). Never call tools.apply_patch directly: managed write guards cannot resolve its target path and fail closed, which wastes the whole invocation.",
