...
export const HOOK_MAP: Record<string, HookMapEntry> = {
  ...
  "superflow-managed-work-guard.sh": {
    event: "PreToolUse",
    matcher:
      "Edit|Write|MultiEdit|NotebookEdit|apply_patch|Bash|Shell|exec_command",
  },
};
...
export function clearSddHooks(settingsFile: string): number {
  ...
  for (const event of Object.keys(settings.hooks)) {
    ...
    const filtered = entries.filter((entry: any) => {
      const cmds: string[] = (entry?.hooks ?? [])
        .map((h: any) => h.command)
        .filter(Boolean);
      const allSuperflow =
        cmds.length > 0 && cmds.every((c) => isSuperflowManagedHook(c));
      if (allSuperflow) {
        removed += cmds.length;
...
export function registerHook(
  ...
  if (!map) {
    throw new Error(
      `Unknown hook script: ${scriptName}. Expected one of: ${Object.keys(HOOK_MAP).join(", ")}`,
    );
  }
  ...
  const exists = settings.hooks[finalEvent].some((entry: any) => {
    const matcherMatches =
      finalMatcher === undefined
        ? entry.matcher === undefined
        : entry.matcher === finalMatcher;
    return (
      entry.hooks?.some((h: any) => h.command === commandPath) && matcherMatches
    );
  });
  if (!exists) {
