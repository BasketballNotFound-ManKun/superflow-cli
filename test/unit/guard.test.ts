...
async function makeCrossServiceChange() {
  ...
  await write(path.join(change, 'api.md'), '# API\n\nPOST /internal/appointment-orders\n');
  await write(
  ...
  await write(path.join(change, 'specs', 'appointment-route', 'spec.md'), '# Spec\n');
  await write(
    path.join(
      change,
      'docs',
...
describe('superflow-guard.sh', () => {
  ...
  it('requires architecture boundary matrix for cross-service design', async () => {
    ...
    await expect(
      ...
      stderr: expect.stringContaining('architecture boundary and call direction matrix'),
    });
  });
