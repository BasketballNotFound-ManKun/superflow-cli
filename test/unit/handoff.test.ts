...
async function makeChangeDir() {
  ...
  await write(
    path.join(change, "specs", "coupon-expiry-reminder", "spec.md"),
    "# Spec\n",
  );
  await write(
    path.join(
...
describe("superflow-handoff.sh", () => {
  beforeEach(async () => {
    tmp = await fs.promises.mkdtemp(
      path.join(os.tmpdir(), "superflow-handoff-"),
    );
  });
  ...
  it("preserves contract design_doc when technical_design exists", async () => {
    ...
    const designDoc = await execFileAsync("bash", [
      STATE,
      "get",
      change,
      "design_doc",
  ...
  it(
    "refreshes docs-only handoff without a technical design directory",
    async () => {
      const change = await makeChangeDir();
