...
describe('core/detect', () => {
  ...
  it('parseAgentSelection 拒绝未知 agent 选择', () => {
    expect(() => parseAgentSelection('unsupported-agent')).toThrow(/claude, codex, both/);
    expect(() => parseAgentSelection('all')).toThrow(/claude, codex, both/);
  ...
  it('resolveAgents 展开 both 为 claude + codex', () => {
    ...
    expect(resolveAgents(['codex', 'claude'])).toEqual(['codex', 'claude']);
  });
});
