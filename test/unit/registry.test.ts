...
describe("core/registry", () => {
  ...
  it("HOOK_MAP 含 11 个 hook 脚本", () => {
    ...
    expect(HOOK_MAP["superflow-managed-work-guard.sh"]).toBeDefined();
  ...
  it("registerHook 添加新 hook 但保留现有 key", () => {
    registerHook(
      ...
      "/home/test/.claude/scripts/superflow-enforce-hook.sh",
    );
  ...
  it("registerHook 支持 UserPromptSubmit 无 matcher hook", () => {
    const cmd =
      "/home/test/.claude/scripts/superflow-dependency-update-hook.sh";
  ...
  it("clearSddHooks 清掉所有 superflow/legacy sdd hook（含 ~ 路径和绝对路径）", () => {
            ...
            PostToolUse: [
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command: "~/.claude/scripts/claude-auto-backup-hook.sh",
                  },
                ],
              },
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command:
                      "/Users/test/.claude/scripts/claude-auto-backup-hook.sh",
                  },
                ],
              },
            ],
            PreToolUse: [
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command: "~/.claude/scripts/superflow-enforce-hook.sh",
                  },
                ],
              },
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command:
                      "/Users/test/.claude/scripts/superflow-enforce-hook.sh",
                  },
                ],
              },
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command: "~/.claude/scripts/superflow-contract-hooks.sh",
                  },
                ],
              },
            ],
            UserPromptSubmit: [
              {
                ...
                hooks: [
                  {
                    type: "command",
                    command: 'echo "non-superflow system prompt"',
                  },
                ],
              },
            ],
    ...
    expect(result.hooks.UserPromptSubmit[0].hooks[0].command).toBe(
      'echo "non-superflow system prompt"',
    );
  });
  ...
  it("clearSddHooks 保留所有顶层 key", () => {
    ...
    const result = JSON.parse(fs.readFileSync(SETTINGS, "utf-8"));
    expect(result.enabledPlugins["superpowers@superpowers-marketplace"]).toBe(
      true,
    );
    expect(result.env.FOO).toBe("bar");
      ...
      for (const entries of Object.values(r.hooks) as any[]) {
        for (const e of entries) {
          for (const h of e.hooks ?? []) {
            if (
              h.command?.includes("/scripts/superflow-") ||
              h.command?.includes("/scripts/sdd-") ||
              h.command?.includes("claude-auto-backup")
            )
              c++;
          }
        }
