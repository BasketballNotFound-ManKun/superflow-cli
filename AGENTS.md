...
以后修改 Superflow CLI，默认先做 ECC 式能力治理检查，不把“新增 Skill / Hook / Prompt”
当作默认解法：

1. 先查 Skill Registry、现有路由、path-scoped rules、Hook、脚本和安装清单，确认唯一
   owner 与规范事实源；能扩展现有能力就不新增平行入口。
2. 新增或修改 Skill 时明确触发/不触发条件、输入输出、相邻能力边界、状态 owner 和
   中英文镜像；运行 `superflow skill-audit --strict`。
3. 确定性事实放 CLI/Hook/门禁，开放式语义留给 Agent；Hook 应范围窄、可诊断，
   advisory 与阻断语义分明，不重复托管状态机的裁决。
4. 安装、更新和迁移保持单一管理者、幂等、备份、来源可追溯；未知或用户自定义资产
   保留，Codex/Claude 双端与项目/全局作用域分别核验。
5. 每项治理变化用代表性正反例核验路由、误报、成本和安装闭环；确认收益后再保留，
   无证据的额外层级、菜单、Hook 和长期状态应撤回。
