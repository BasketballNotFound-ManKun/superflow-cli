...
项目级 init（非 dry-run）及创建托管任务时追加 `.superflow/` 忽略规则；自动写入后记录工作区基线，后续 `.gitignore` 修改仍参与审计。交付终态自动执行既有留存清理；阻塞和运行中的任务保留恢复现场。清理保护当前交接、报告与事件引用的证据；失败记录 `run.retention_deferred`，不伪造删除成功，也不撤销有效交付。

环境不可达报告先脱敏保存原始日志，停止自动整改并由 Host 核实；关键词只用于路由，不证明报告真实。环境恢复后通过既有用户指引恢复原任务，继承有效证据。
